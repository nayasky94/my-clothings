...
type SearchResultItem = Awaited<
  ReturnType<typeof notion.search>
>['results'][number]
...
function getStringProp(
  ...
  if (
    typeof prop === 'object' &&
    prop !== null &&
    (prop as Record<string, unknown>).type === type
  ) {
    return prop
  }
  return null
...
function extractName(props: Record<string, unknown>): string {
  const prop = getStringProp(props, 'Title', 'title') as {
...
function extractSelect(props: Record<string, unknown>, key: string): string {
  const prop = getStringProp(props, key, 'select') as {
...
function extractMultiSelect(
  ...
  const prop = getStringProp(props, key, 'multi_select') as {
...
function extractDate(
  ...
  const prop = getStringProp(props, key, 'date') as {
    date: { start: string } | null
  } | null
  return prop?.date?.start ?? null
}

export async function getClothingList(): Promise<Clothing[]> {
  ...
  return response.results
    .filter(isFullPage)
    .map(item => {
      const page = item as SearchResultItem & {
